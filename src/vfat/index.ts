...
import { getBscPrices, getBscToken } from "./bsc_helper";
import { getPoolPrices, init_ethers, printChefPool } from "./ethers_helper";
...
async function loadBscAUTOChefContract(
	...
	const poolInfos = await Promise.all(
		Array.from(Array(poolCount).keys()).map(
			async (x) => await getBscAUTOPoolInfo(App, chefContract, x, pendingRewardsFunction)
		)
	);
