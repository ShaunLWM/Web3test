...
export async function init_ethers() {
	const App = {
		YOUR_ADDRESS: "0x0895196562C7868C5Be92459FaE7f877ED450452",
...
const _print_inline = function (message) {
	...
	for (let i = 0; i < arguments.length; i++) {
		if (typeof arguments[i] == "object") {
			console.log(JSON && JSON.stringify ? JSON.stringify(arguments[i], undefined, 2) : arguments[i]);
		} else {
...
const _print_bold = function (message) {
	for (let i = 0; i < arguments.length; i++) {
		if (typeof arguments[i] == "object") {
			console.log("<b>" + (JSON && JSON.stringify ? JSON.stringify(arguments[i], undefined, 2) : arguments[i]) + "</b><br />");
		} else {
			console.log("<b>" + arguments[i] + "</b><br />");
		}
	}
};
...
const _print_href = function (message, href) {
	const uuid = ID();
	console.log(`<a href="${href}" target="_blank" id="${uuid}">${message}</a><br />`);
};
...
export const lookUpPrices = async function (id_array) {
	...
	for (const id_chunk of chunk(id_array, 50)) {
		...
		let json = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=" + ids + "&vs_currencies=usd");
...
function getWrapPrices(tokens, prices, pool) {
	...
	if (wrappedToken.token0 != null) {
		...
		const price =
			((pool.balance / 10 ** wrappedToken.decimals) * tokenPrice) / (pool.totalSupply / 10 ** pool.decimals);
		const tvl = (pool.balance / 10 ** wrappedToken.decimals) * price;
		const staked_tvl = pool.staked * price;
		prices[pool.address] = { usd: price };
		return {
