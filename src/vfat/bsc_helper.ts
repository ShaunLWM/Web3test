import { getPoolPrices, init_ethers, lookUpPrices, lookUpTokenPrices, printChefPool } from "./ethers_helper";
import { ethers } from "ethers";
import { CURVE_ABI, ERC20_ABI, MINTER_ABI, UNI_ABI } from "./data";
...
const getItem = (address: string) => {
	console.log(`Fetching from localStorage ${address}`);
	return storage[address];
};

const setItem = (address: string, value: string) => {
	console.log(`Setting ${value} to ${address}`);
...
export async function getBscToken(App, tokenAddress, stakingAddress) {
	...
	const type = getItem(tokenAddress);
...
export async function loadBscSynthetixPoolInfo(
	App,
	tokens,
	prices,
	stakingAbi,
